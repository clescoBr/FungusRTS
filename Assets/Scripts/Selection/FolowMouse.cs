...
public class FollowMouse2D : MonoBehaviour
{
    ...
    private bool mouseRight; // right clicked?
    ...
    public GameObject mycelium; // start infecting tile
    ...
    private void SetupActions()
    {
        ...
        mouseRightCl.canceled += Mouse_right_canceled;
    ...
    private void Mouse_right_click(InputAction.CallbackContext obj) // if rightClicked
    {
        ...
        {
            mouseRight = true; // if right mouse was pressed

            if (overlord.GetComponent<OvermindController>().selected == true && // if overlord curently left clicked
                abilityTarget.GetComponent<TileIdentity>().tileType == "PlInfected" )  // if the target tile is captured
            {
    ...
    /// records right mouse press
    /// </summary>
    private void Mouse_right_canceled(InputAction.CallbackContext obj) // right release
    {
        mouseRight = false;
    ...
    private void InfectTile(GameObject tile)
    {
        ...
        GameObject spawn = Instantiate(mycelium, new Vector3 (tile.transform.position.x, 
    ...
    /// <summary>
    ///     /// when the mouse pointer exits something return it to original properties
    /// </summary>
